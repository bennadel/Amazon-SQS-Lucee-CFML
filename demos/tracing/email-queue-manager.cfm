<cfscript>
	...
	// any given request will almost always be overlapping with a previous, still-
	// executing request. As such, let's use a no-error lock to synchronize our
	// monitoring of our Amazon SQS queue, email-queue.
	lock
		...
		systemOutput( "Start polling email-queue for new messages.", true );
		...
		while ( getTickCount() <= maxTickCount ) {
			...
			systemOutput( "LOOP: Checking email-queue for new messages.", true );

			application.emailQueueService.processNewMessages();
	...
	if ( ! cflock.succeeded ) {
		...
		systemOutput( "Lock failure, email-queue monitoring already in place.", true, true );
